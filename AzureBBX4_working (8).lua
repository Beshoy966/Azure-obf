...
local Library = {
    _config = (function()
        local cfg = Config:load(game.GameId)
        if cfg and cfg._flags then
            cfg._flags["ZX_PlayerAim"] = nil
            cfg._flags["ZX_PlayerAimNotify"] = nil
            cfg._flags["ZX_PlayerAimTarget"] = nil
        end
        return cfg
    end)(),
...
local EspTab = library:create_tab("ESP")
    ...
    callback = function(state)
        ...
        if state then
            ...
            if System.__properties.__connections.__auto_rewards then
                ...
                System.__properties.__connections.__auto_rewards = nil
            end
        end
    end
})
...
function System.curve.get_cframe()
    ...
    local function get_smart_dot(distance, target_velocity_mag)
        local base = 0.85
        local distance_factor = math.clamp(distance / 500, 0, 1) * 0.10
        local velocity_factor = math.clamp(target_velocity_mag / 50, 0, 1) * 0.05
        return math.clamp(base + distance_factor - velocity_factor, 0.75, 0.97)
    end

    local function get_smart_offset(distance)
        local scale = math.clamp(distance * 0.5, 50, 2000)
        return Vector3.new(
            math.random(-scale, scale),
            math.random(-scale * 0.5, scale * 0.5),
            math.random(-scale, scale)
        )
    end
    ...
    local target_distance = (target_pos - root.Position).Magnitude
    local target_velocity = targetPart and targetPart.AssemblyLinearVelocity or Vector3.zero
    local smart_threshold = get_smart_dot(target_distance, target_velocity.Magnitude)
    ...
    local curve_functions = {
        ...
        function()
            ...
            local random_offset
            local best_offset, best_dot = nil, math.huge
            local attempts = 0

            repeat
                random_offset = get_smart_offset(target_distance)
                ...
                attempts = attempts + 1
                if dot < smart_threshold then break end
            until attempts > 15

            if best_dot >= smart_threshold then
                random_offset = best_offset
            end
        ...
        function()
            local offset = math.clamp(target_distance * 0.02, 3, 15)
            return CFrame.new(root.Position, target_pos + Vector3.new(0, offset, 0))
        end,

        function()
            local direction = (root.Position - target_pos).Unit
            local back_dist = math.clamp(target_distance * 2, 200, 10000)
            local backwards_pos = root.Position + direction * back_dist + Vector3.new(0, 1000, 0)
        ...
        function()
            local left_vec = -camera.CFrame.RightVector * math.max(target_distance * 2, 500)
            return CFrame.new(root.Position, root.Position + left_vec)
        end,

        function()
            local right_vec = camera.CFrame.RightVector * math.max(target_distance * 2, 500)
            return CFrame.new(root.Position, root.Position + right_vec)
        end,

        function()
            local Aimed_Player, Closest_Distance = nil, math.huge
...
-- Resets when cooldown finishes (Offset.Y >= 0.985).
task.spawn(function()
    local InfinityCD
    pcall(function()
        InfinityCD = LocalPlayer.PlayerGui.Hotbar.Ability.Duration.Fill.UIGradient
    end)
    if not InfinityCD then return end
    InfinityCD:GetPropertyChangedSignal("Offset"):Connect(function()
        ...
        System.__properties.__plr_forc = false
        if abilities:FindFirstChild("Forcefield") and abilities["Forcefield"].Enabled and detections.__forcefield then
            System.__properties.__plr_forc = true
        elseif abilities:FindFirstChild("Slashes of Fury") and abilities["Slashes of Fury"].Enabled and detections.__slashesoffury then
            System.__properties.__plr_forc = true
        elseif abilities:FindFirstChild("Time Hole") and abilities["Time Hole"].Enabled and detections.__timehole then
            System.__properties.__plr_forc = true
        elseif abilities:FindFirstChild("Death Slash") and abilities["Death Slash"].Enabled and detections.__deathslash then
            System.__properties.__plr_forc = true
        elseif abilities:FindFirstChild("Infinity") and abilities["Infinity"].Enabled and detections.__infinity then
            System.__properties.__plr_forc = true
        end
        if InfinityCD.Offset.Y >= 0.985 then
            System.__properties.__plr_forc = false
        end
    end)
end)

-- Anti Phantom Attack (ported from AHHX)
-- When Phantom attack is detected on local player, auto-move toward ball
-- to intercept it. Resets when ball target changes.
...
getgenv().AntiPhantomAttack = getgenv().AntiPhantomAttack or false
...
autoparry_module:create_checkbox({
    title = "Anti Phantom Attack",
    flag = "AntiPhantomAttack",
    callback = function(value)
        getgenv().AntiPhantomAttack = value
    end
})
...
local lobby_ap_module = AutoparryTab:create_module({
    title = "Lobby AP",
    description = "Auto parries ball in lobby/training",
    flag = "LobbyAPModule",
    section = "right",
    callback = function(state)
        getgenv().LobbyAPEnabled = state
        if state then
            ...
            if System.__properties.__connections.__lobby_ap then
                System.__properties.__connections.__lobby_ap:Disconnect()
                System.__properties.__connections.__lobby_ap = nil
            end
            getgenv()._ZX_LobbyAP_ParryCooldown = false
            if getgenv().LobbyAPNotify then
                send_notification("Lobby AP", "OFF", 2)
            end
        end
    end
})
...
lobby_ap_module:create_checkbox({
    ...
    callback = function(value)
        getgenv().LobbyAPRandomAccuracy = value
    end
})
    ...
    callback = function(state)
        if getgenv().ManualSpamNotify then
            if state then
                ...
                if not getgenv()._ZX_MSFloatBtn then
                    local PlayerGui = LocalPlayer:WaitForChild("PlayerGui")
                    pcall(function()
                        local old = PlayerGui:FindFirstChild("ZX_ManualSpamPanel")
                        if old then old:Destroy() end
                    end)
                    local MSPanel = Instance.new("ScreenGui")
                    MSPanel.Name = "ZX_ManualSpamPanel"
                    MSPanel.Parent = PlayerGui
                    MSPanel.ResetOnSpawn = true
                    MSPanel.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
                    local MSFrame = Instance.new("Frame")
                    MSFrame.Name = "MSFrame"
                    MSFrame.Parent = MSPanel
                    MSFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
                    MSFrame.BackgroundTransparency = 0.3
                    MSFrame.BorderSizePixel = 0
                    MSFrame.Position = UDim2.new(0.057, 0, 0.2, 0)
                    MSFrame.Size = UDim2.new(0, 139, 0, 89)
                    local UICorner = Instance.new("UICorner")
                    UICorner.Parent = MSFrame
                    local MSTitle = Instance.new("TextLabel")
                    MSTitle.Name = "Title"
                    MSTitle.Parent = MSFrame
                    MSTitle.BackgroundTransparency = 1
                    MSTitle.Size = UDim2.new(0, 139, 0, 29)
                    MSTitle.Font = Enum.Font.SourceSans
                    MSTitle.Text = "Manual Spam"
                    MSTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
                    MSTitle.TextScaled = true
                    MSTitle.TextWrapped = true
                    local MSActivation = Instance.new("TextButton")
                    MSActivation.Name = "Activation"
                    MSActivation.Parent = MSFrame
                    MSActivation.BackgroundTransparency = 1
                    MSActivation.Position = UDim2.new(0, 0, 0.326, 0)
                    MSActivation.Size = UDim2.new(0, 139, 0, 60)
                    MSActivation.Font = Enum.Font.SourceSans
                    MSActivation.Text = "OFF"
                    MSActivation.TextColor3 = Color3.fromRGB(255, 0, 0)
                    MSActivation.TextScaled = true
                    MSActivation.TextWrapped = true
                    local msActive = false
                    MSActivation.MouseButton1Click:Connect(function()
                        msActive = not msActive
                        if msActive then
                            MSActivation.Text = "ON"
                            MSActivation.TextColor3 = Color3.fromRGB(0, 255, 0)
                    ...
                    local function update(input)
                        ...
                        TweenService:Create(MSFrame, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = newPos}):Play()
                    end
                    MSFrame.Active = true
                    MSFrame.InputBegan:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                            dragging = true
                            dragStart = input.Position
                            startPos = MSFrame.Position
                            input.Changed:Connect(function()
                                if input.UserInputState == Enum.UserInputState.End then dragging = false end
                            end)
                        end
                    end)
                    MSFrame.InputChanged:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
                            dragInput = input
                        end
                    end)
                    ...
                    getgenv()._ZX_MSFloatBtn = MSPanel
                end
                getgenv()._ZX_MSFloatBtn.Enabled = true
            else
                pcall(function()
                    Library.SendNotification({
                        title = "Manual Spam",
                ...
                if getgenv()._ZX_MSFloatBtn then
                    getgenv()._ZX_MSFloatBtn.Enabled = false
                end
            end
        end
...
manual_spam_module:create_checkbox({
    title = "UI",
    flag = "ZX_MS_Panel_Toggle",
    callback = function(value)
        getgenv()._ZX_MSPanelEnabled = value
        if value then
            if not getgenv()._ZX_MSPanel then
                local PlayerGui = LocalPlayer:WaitForChild("PlayerGui")
                pcall(function()
                    local old = PlayerGui:FindFirstChild("ZX_ManualSpamPanel")
                    if old then old:Destroy() end
                ...
                getgenv()._ZX_MSPanel = MSPanel
            end
            getgenv()._ZX_MSPanel.Enabled = true
        else
            if getgenv()._ZX_MSPanel then
                getgenv()._ZX_MSPanel.Enabled = false
            end
        end
    end
})
...
manual_spam_module:create_checkbox({
    ...
    callback = function(value)
        ...
        if value and not getgenv()._ZX_AnimFixConn then
            local net_table_lit = {}
            ...
            if get_remote_not then
                getgenv()._ZX_AnimFixConn = RunService.Heartbeat:Connect(function()
                    if not getgenv().ManualSpamAnimationFix and not getgenv().AutoSpamAnimationFix then return end
                    pcall(function()
                        get_remote_not:FireServer(
                            ...
                            (function()
                                local get_t_net = ""
                                local ts = tostring(math.floor(workspace:GetServerTimeNow() * 100))
                                local key_result = get_key_net_time(get_num_net, "TIME")
                                for i = 1, #ts do
                                    get_t_net = get_t_net .. string.char(bit32.bxor(
                                        (ts:byte(i) + i) % 256,
                                        key_result:byte((i - 1) % #key_result + 1)
                                    ))
                                end
                                pcall(function()
                                    local conns = getconnections(game.Players.LocalPlayer.PlayerGui.Hotbar.Block.Activated)
        ...
        elseif not value and getgenv()._ZX_AnimFixConn then
            getgenv()._ZX_AnimFixConn:Disconnect()
            getgenv()._ZX_AnimFixConn = nil
        end
    end
})
...
local ability_esp_module = EspTab:create_module({
    title = "Ability ESP",
...
getgenv()._ZX_BallESP = EspTab:create_module({
    ...
    section = "right",
    callback = function(state)
        getgenv().BallESPEnabled = state
        if state then
            if not Connections_Manager["BallESP"] then
                Connections_Manager["BallESP"] = RunService.RenderStepped:Connect(function()
                    local ball = System.ball.get()
                    if not ball then return end
                    local zoomies = ball:FindFirstChild("zoomies")
                    if not zoomies then return end
...
getgenv()._ZX_BallTracer = EspTab:create_module({
    ...
    section = "right",
    callback = function(state)
        getgenv().BallTracerEnabled = state
        if state then
            if not Connections_Manager["BallTracer"] then
...
getgenv()._ZX_ShieldChanger = EspTab:create_module({
    ...
    section = "left",
    callback = function(state)
        getgenv().ShieldChangerEnabled = state
    end
})
...
getgenv()._ZX_SlashChanger = EspTab:create_module({
    ...
    section = "left",
    callback = function(state)
        getgenv().SlashChangerEnabled = state
    end
})
...
task.spawn(function()
    RunService.RenderStepped:Connect(function()
        if not getgenv().ShieldChangerEnabled and not getgenv().SlashChangerEnabled then return end
        local char = LocalPlayer.Character
        if not char then return end
        for _, v in ipairs(char:GetDescendants()) do
            if v:IsA("ParticleEmitter") or v:IsA("Trail") or v:IsA("Beam") then
                if getgenv().ShieldChangerEnabled and getgenv().ShieldColor and v.Name:lower():find("shield") then
                    pcall(function() v.Color = ColorSequence.new(getgenv().ShieldColor) end)
                end
                if getgenv().SlashChangerEnabled and getgenv().SlashColor and (v.Name:lower():find("slash") or v.Name:lower():find("hit")) then
                    pcall(function() v.Color = ColorSequence.new(getgenv().SlashColor) end)
                end
            end
        end
    end)
...
getgenv()._ZX_PullDetection = DetectionTab:create_module({
    ...
    callback = function(state)
        ...
        if System and System.__config and System.__config.__detections then
            System.__config.__detections.__pull = state
        end
    end
})
end)()
